...
const api = {
   ...
   * @param {string} model - Gemini model to use (optional, defaults to gemini-3.1-flash-lite-preview)
   */
  async chat(message, sessionId, model = 'gemini-3.1-flash-lite-preview') {
  ...
  /**
   * Preview the deterministic subagent workflow for a query.
   * @param {string} query - User query
   */
  async previewWorkflow(query) {
    const response = await fetchWithRetry(
      `${API_BASE}/api/workflow/preview`,
      {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ detail: response.statusText }));
      throw new Error(error.detail || 'Workflow preview failed');
    }
  ...
  /**
   * Get live subagent workflow state for a chat session.
   * @param {string} sessionId - Session ID
   */
  async getSessionWorkflow(sessionId) {
    const response = await fetchWithRetry(
      `${API_BASE}/api/session/${sessionId}/workflow`,
      {},
      { timeout: 10000 }
    );
    ...
    if (!response.ok) {
      const error = await response.json().catch(() => ({ detail: response.statusText }));
      throw new Error(error.detail || 'Failed to load workflow state');
    }
  ...
  async getMlPolymerTypes() {
    const response = await fetchWithRetry(`${API_BASE}/api/ml/polymer-types`);
    if (!response.ok) {
      const error = await response.json().catch(() => ({ detail: response.statusText }));
      throw new Error(error.detail || 'Failed to load polymer types');
    }
    return response.json();
  },

  async getMlPolymersByType(polymerType) {
    const response = await fetchWithRetry(
      `${API_BASE}/api/ml/polymers-by-type/${encodeURIComponent(polymerType)}`
    );
    if (!response.ok) {
      const error = await response.json().catch(() => ({ detail: response.statusText }));
      throw new Error(error.detail || 'Failed to load polymers');
    }
    return response.json();
  },
