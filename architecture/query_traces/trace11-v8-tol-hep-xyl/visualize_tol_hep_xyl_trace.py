...
3 x biosteam-analyst running Toluene, Heptane, Xylene in parallel.

Trace ID: 019c58d4-4ebd-72a3-80d7-190140b884fe
"""
...
C_BIO3       = "#0EA5E9"   # Sky — biosteam-analyst #3 (Xylene)
...
def create_trace_figure():
    ...
    query = (
        ...
        "parallel: Toluene, Heptane, and Xylene. Run BioSTEAM simulations "
        "for each under energy case C1 and report MSP, GWP, and TCI side "
        "by side."
    )
    box_h = 0.052
    ...
    ax.text(cl, y - 0.022, _wrap(query, 115),
            fontsize=9, color=C_BODY, transform=ax.transAxes,
    ...
    for a in agents:
        ...
        ax.text(a_cr, cy, f'{a["time"]}  |  {a["tokens"]}',
                fontsize=9, color=C_BODY, ha="right",
                transform=ax.transAxes, va="top", zorder=5)
        cy -= 0.016
        ...
        ax.text(a_cl, cy, "Tool Calls (1 domain):",
                fontsize=9, fontweight="bold", color=C_TOOL_TEXT,
                transform=ax.transAxes, va="top", zorder=5)
        cy -= 0.019

        _pill(ax, a_cl, cy, a["tool"], C_TOOL_TEXT, C_TOOL_BG, C_TOOL_TEXT, fs=7)
        cy -= PILL_H + 0.004
        ...
        ax.text(a_cl, cy, "Results:",
                fontsize=9, fontweight="bold", color=a["output_color"],
                transform=ax.transAxes, va="top", zorder=5)
        cy -= 0.014
        ax.text(a_cl, cy,
                f'MSP: {a["msp"]}\n'
    ...
    ax.text(cl, y - 0.010, "Orchestrator Synthesis",
            ...
            transform=ax.transAxes, va="top", zorder=5)
    ax.text(cr, y - 0.010, "10.5s  |  verifier: PASS",
            fontsize=9, color=C_VERIFIER, ha="right", fontweight="bold",
            transform=ax.transAxes, va="top", zorder=5)
    ...
    for i, (solvent, msp, gwp, tci) in enumerate(rows):
        ...
        for cx, val in zip(col_positions, vals):
            ax.text(cx, ry, val, fontsize=9, color=rank_colors[i],
                    fontweight="bold" if i == 0 else "normal",
                    fontfamily="monospace" if cx != tab_left else "sans-serif",
                    transform=ax.transAxes, va="top", zorder=5)

    # Winner callout
    ax.text(cl, tab_top - 0.060,
            "Winner: Xylene  --  lowest MSP ($0.982/kg) and TCI ($71.2M);  "
            "Heptane has lowest GWP",
            fontsize=9, color="#15803D", fontweight="bold", style="italic",
            transform=ax.transAxes, va="top", zorder=5)
    ...
    # Row labels
    row_labels = ["Toluene", "Heptane", "Xylene"]
    row_colors = [C_BIO1, C_BIO2, C_BIO3]

    # Timing: route ~13s, then 3 parallel sims ~16s each, synthesis ~10.5s
    route_dur = 13.0
    sim_durs = [16.0, 16.2, 15.9]
    ...
    for i in range(3):
        ...
        ax.text(bar_left - 0.005, by + bar_h / 2, row_labels[i],
                ha="right", va="center", fontsize=9, color=row_colors[i],
    ...
    # Time axis labels
    ty = rows_y[2] - 0.012
    for t in [0, 10, 20, 30, 40, 50, 60, 67.6]:
    ...
    items = [(C_USER, "Orchestrator"), (C_BIO1, "Toluene"),
             (C_BIO2, "Heptane"), (C_BIO3, "Xylene"), (C_SYNTH, "Synthesis")]
    lx = bar_left + 0.01
    ...
    for color, label in items:
        ax.plot([lx, lx + 0.016], [ly, ly], color=color, lw=5,
                ...
                zorder=3, alpha=0.85)
        ax.text(lx + 0.022, ly, label, fontsize=9, color=C_BODY,
    ...
    for label, value in metrics:
        ax.text(p1x + pcl, my, label, fontsize=9, color=C_BODY,
                transform=ax.transAxes, va="center", zorder=5)
        ax.text(p1x + pw - 0.006, my, value, fontsize=9,
    ...
    ax.text(MID, y,
            ...
            "Trace 019c58d4  |  "
            "Toluene vs Heptane vs Xylene — Parallel BioSTEAM",
            ha="center", fontsize=9, color=C_BODY,
            transform=ax.transAxes)

    # ── Save ──
    out_dir = os.path.dirname(os.path.abspath(__file__))
    out_path = os.path.join(out_dir, "trace11-v8-tol-hep-xyl.png")
