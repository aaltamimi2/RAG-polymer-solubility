#!/usr/bin/env python
"""Generate summary visualization for parallel trace campaign.

Reads the results JSON produced by run_parallel_traces.py and
generates a publication-quality summary figure showing all 10 queries.

Usage:
    python visualize_parallel.py                          # latest results
    python visualize_parallel.py results_20260224_*.json  # specific file
"""
...
def draw_summary(results: list[dict], output_path: str, campaign: str = "Parallel"):
    n = len(results)
    ...
    ax.text(TOKEN_X, col_y, "Tokens", fontsize=7, fontweight="bold",
            color=C_BODY, transform=ax.transAxes, va="center")
    ax.text(AGENTS_X, col_y, "Parallel Pair", fontsize=7, fontweight="bold",
            color=C_BODY, transform=ax.transAxes, va="center")
    ...
    for i, r in enumerate(results):
        ...
        # Wall-time bar — overlapping segments to show parallelism
        ...
        # Overlapping bars to visualize parallel execution
        n_agents = max(len(actual), 1)
        for j, sa in enumerate(actual):
            color = _agent_color(sa)
            offset = j * bar_h_frac * 0.15  # slight vertical offset
            ax.add_patch(FancyBboxPatch(
                (BAR_LEFT, row_y - bar_h_frac / 2 + offset),
                bar_w, bar_h_frac * 0.7,
                boxstyle="round,pad=0,rounding_size=0.003",
                facecolor=color, edgecolor="none",
                alpha=0.65 if j > 0 else 0.80,
                transform=ax.transAxes, zorder=2 + j,
            ))
        ...
        # Subagent parallel pills with || separator
        px = AGENTS_X
        for k, sa in enumerate(actual):
            color = _agent_color(sa)
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize parallel trace campaign results"
    )
    ...
    output = args.output or str(_DIR / "summary_parallel.png")
    campaign = data.get("campaign", "Parallel")
