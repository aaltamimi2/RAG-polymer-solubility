#!/usr/bin/env python
"""Generate summary visualization for sequential trace campaign.

Reads the results JSON produced by run_sequential_traces.py and
generates a publication-quality summary figure showing all 10 queries.

Usage:
    python visualize_sequential.py                          # latest results
    python visualize_sequential.py results_20260224_*.json  # specific file
"""
...
def draw_summary(results: list[dict], output_path: str, campaign: str = "Sequential"):
    n = len(results)
    ...
    ax.text(TOKEN_X, col_y, "Tokens", fontsize=7, fontweight="bold",
            color=C_BODY, transform=ax.transAxes, va="center")
    ax.text(AGENTS_X, col_y, "Subagent Chain", fontsize=7, fontweight="bold",
            color=C_BODY, transform=ax.transAxes, va="center")
    ...
    for i, r in enumerate(results):
        ...
        # Stacked bar: split proportionally per subagent
        n_agents = max(len(actual), 1)
        seg_w = bar_w / n_agents
        for j, sa in enumerate(actual):
            color = _agent_color(sa)
            ax.add_patch(FancyBboxPatch(
                (BAR_LEFT + j * seg_w, row_y - bar_h_frac / 2),
                seg_w, bar_h_frac,
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize sequential trace campaign results"
    )
    ...
    output = args.output or str(_DIR / "summary_sequential.png")
    campaign = data.get("campaign", "Sequential")
