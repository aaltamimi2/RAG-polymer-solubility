...
from strap.services.rag_service import (
    analyze_dense_vs_sparse as analyze_dense_vs_sparse_report,
    analyze_query_expansion as analyze_query_expansion_report,
    analyze_retrieval_patterns as analyze_retrieval_patterns_report,
    analyze_reranking_impact as analyze_reranking_impact_report,
    analyze_search_scores,
    analyze_section_boost_impact,
    compute_document_similarity_matrix,
    visualize_embedding_space as visualize_embedding_space_report,
)
...
async def analyze_search_diagnostics(
    ...
    """
    try:
        analysis = analyze_search_scores(query=query, top_k=top_k)
...
async def visualize_retrieval_patterns() -> str:
    ...
    """
    try:
        analysis = analyze_retrieval_patterns_report()
        if analysis.get("error"):
            return f"❌ {analysis['error']}"
...
async def visualize_embedding_space(
    ...
    """
    try:
        analysis = visualize_embedding_space_report(sample_size=sample_size, method=method)
        if analysis.get("error"):
            return f"❌ {analysis['error']}"
...
async def analyze_document_similarity() -> str:
    ...
    """
    try:
        analysis = compute_document_similarity_matrix()
        if analysis.get("error"):
            return f"❌ {analysis['error']}"
...
async def analyze_dense_vs_sparse() -> str:
    ...
    """
    try:
        analysis = analyze_dense_vs_sparse_report()
        if analysis.get("error"):
            return f"❌ {analysis['error']}"
...
async def analyze_reranking_impact() -> str:
    ...
    """
    try:
        analysis = analyze_reranking_impact_report()
        if analysis.get("error"):
            return f"❌ {analysis['error']}"
...
async def analyze_section_boost() -> str:
    ...
    """
    try:
        analysis = analyze_section_boost_impact()
        if analysis.get("error"):
            return f"❌ {analysis['error']}"
...
async def analyze_query_expansion() -> str:
    ...
    """
    try:
        analysis = analyze_query_expansion_report()
        if analysis.get("error"):
            return f"❌ {analysis['error']}"
