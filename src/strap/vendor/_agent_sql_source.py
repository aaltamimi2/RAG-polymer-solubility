...
async def plan_sequential_separation(
    ...
    - excluded_solvents: Comma-separated list of solvents to exclude (e.g., for cost constraints)
    ...
    async def find_top_solvents(target: str, remaining: list, k: int = 5, used_solvents: set = None, excluded_solvents: set = None) -> list:
        ...
        Args:
            ...
            remaining: Remaining polymers in the mixture
            k: Number of top solvents to return
            used_solvents: Solvents already used in previous separation steps
            excluded_solvents: Solvents to exclude (e.g., from feedback loop cost constraints)
        """
        if used_solvents is None:
            used_solvents = set()
        if excluded_solvents is None:
            excluded_solvents = set()
        ...
        # Filter out expensive solvents from feedback loop (cost optimization)
        if excluded_solvents:
            excluded_lower = {s.lower() for s in excluded_solvents}
            filtered_results = [r for r in results if r["solvent"].lower() not in excluded_lower]
            if filtered_results:
                results = filtered_results
                for r in results[:3]:  # Mark first few as feedback-constrained
                    r["feedback_constrained"] = True
            else:
                # Mark excluded solvents if no alternatives available
                for r in results:
    ...
    async def analyze_sequence(sequence, seq_idx):
        ...
        for step, target in enumerate(sequence[:-1], 1):
            ...
            # Find top solvents, excluding those already used AND those from feedback exclusion
    ...
    structured_data = {
        ...
        "feedback_iteration": len(excluded_set) > 0,  # True if re-running with exclusions
        "steps": [
            {
                "step": i + 1,
                "target": s.get("target", ""),
...
def get_solvent_properties(solvent_names: str) -> str:
    ...
    solvent_data = []
    for _, row in df.iterrows():
        solvent_entry = {"name": row.get(name_col, "Unknown")}
...
async def sql_agent_node(state: AgentState):
    ...
    try:
        ...
        # Use specialist prompt if provided, otherwise default SQL_AGENT_PROMPT
        specialist_prompt = state.get("specialist_prompt")
        if specialist_prompt:
            prompt = specialist_prompt if isinstance(specialist_prompt, str) else str(specialist_prompt)
            logger.info(f"Using specialist prompt ({len(prompt)} chars)")
        else:
